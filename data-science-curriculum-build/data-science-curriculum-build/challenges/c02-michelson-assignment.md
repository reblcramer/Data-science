...
- all values in the example table are rounded to the nearest 10
...
**Observations**: - Write your observations here! - While the rounded
values of mean velocity in 2&3 distinctness samples is equal, the
unrounded mean is slightly higher in 3 distinctness. - Why might your
table differ from Michelson’s? - all values in Michelson’s are rounded
to the nearest 10
...
df_michelson$VelocityVacuum<-df_michelson$Velocity + 92
df_q2 <- df_michelson
df_q2
```
...
## Your code here!
err<-LIGHTSPEED_MICHELSON-LIGHTSPEED_VACUUM
err
```
...
between Michelson’s uncertainty and his error. - Michelson’s error was
about 3 times greater than his uncertainty
...
**Observations**: Similarities - Dates of high and low peaks of the
graphs are similar Differences - The height of the peaks on the graphs
is different. The simulated values are between the dotted lines more of
the time than the real values.
...
**Observations**: I can’t figure a pattern within the data that would
help explain the difference between Michelson’s estimate and
LIGHTSPEED_VACUUM. It may be measurement error or a variable that wasn’t
measured at the time such as humidity - Make sure to record observations
on your graphs!
...
df_michelson%>%
  ggplot()+
  geom_point(aes(Date,Velocity, color = "In Air"))+
  geom_point(aes(Date,VelocityVacuum, color="In Vacuum"))
```
