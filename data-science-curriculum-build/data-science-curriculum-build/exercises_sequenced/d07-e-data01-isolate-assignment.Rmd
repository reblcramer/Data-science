...
df_q3 <- 
  flights %>% 
  filter(arr_delay<=10&arr_delay>=-10&arr_delay!=0
  )
...
= instead of ==, and LAX not in quotes, trying to find all data for LAX
...
either the variable BOS should contain the string 'BOS' or there need to be quotes around BOS in the function to call the sting rather than the variable
...
```{r q6-task}
df_q6a <-flights %>% filter( dest == 'JFK'| dest == 'LGA'|dest == 'EWR')
df_q6b <- flights %>% filter(origin=='JFK'|origin=='LGA'|origin=='EWR')
```
...
- Was this dataset assembled to study flights *out of* NYC, or *into* NYC? How do you know?
  - a contains flights into NYC b contains flights out of NYC
...
longest flights are all to HNL 
```{r q7-task}
